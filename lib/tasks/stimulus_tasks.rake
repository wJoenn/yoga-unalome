...
namespace :stimulus_custom do
  ...
  namespace :manifest do
    task :display do
      puts Stimulus::Manifest.generate_from(Rails.root.join(Dir.glob("**/javascript/controllers").first))
    end

    task :update do
      manifest =
        Stimulus::Manifest.generate_from(Rails.root.join(Dir.glob("**/javascript/controllers").first))

      File.open(Rails.root.join(Dir.glob("**/javascript/controllers/index.js").first), "w+") do |index|
